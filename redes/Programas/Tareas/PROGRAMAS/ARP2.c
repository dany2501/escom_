...
void impTrama(unsigned char *paq, long segs){
	...
	printf("\nTiempo de respuesta: %ld milisegs",segs);
}
...
void enviaTrama(int ds,int index,unsigned char *paq){	
	...
	if(tam==-1)
		...
		numero++;
		printf("\nExito al enviar la trama %d",numero);
	}
}
...
//void guardar(long segs, char nombre[]){
/*
	char c=segs+'0';
	FILE * archivo;

	char cad[]= "Tiempo de respuesta: ";
	...
	fwrite(tramaARPresp,sizeof(char),sizeof(tramaARPresp),archivo);
	fwrite(cad,1,sizeof(cad),archivo);
	fwrite(c,sizeof(char),sizeof(cad),archivo);

	fclose(archivo);
-------------------------------------
char c=segs+'0';
int ar;
	char *nom=nombre,cad[]={"Tiempo de respuesta: "};

	ar=open(nom, O_CREAT | O_APPEND | O_WRONLY,0777);

	write(ar, &tramaARPresp, sizeof(tramaARPresp));
	write(ar, &cad, sizeof(cad));
	write(ar, &c, sizeof(segs));
...
/*
	...
	long tiempo = 0, seconds, useconds, bandera = 0;
...
int main(int argc, char *argv[]){
	...
	*/
	if(argc != 2){
        printf("\nSe requiere el nombre del archivo txt para guardar\n\n");
        exit(0);
    }
